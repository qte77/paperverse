...
export interface SceneHandle {
  ...
  /** Snap the camera to look straight down a world axis at a bounding sphere. */
  viewAxis(
    axis: "x" | "y" | "z",
...
export async function createScene(canvas: HTMLCanvasElement): Promise<SceneHandle> {
  ...
  return {
    ...
    frameSphere(center: readonly [number, number, number], radius: number): void {
      ...
      // Restore the canonical up so Reset returns level even after an axis snap
      // (viewAxis can leave a top-down up-vector behind).
      camera.up.set(0, 1, 0);
    ...
    viewAxis(
      ...
      // Same fit distance as frameSphere (mirrored in renderer.axisViewpoint), but
